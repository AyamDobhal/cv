      ...
      <section className="mx-auto w-full max-w-2xl space-y-8   print:space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex-1 space-y-1.5">
            ...
            <div className="hidden flex-col gap-x-1 font-mono text-sm  print:flex print:text-[12px]">
              {RESUME_DATA.contact.email ? (
                <a href={`mailto:${RESUME_DATA.contact.email}`}>
        ...
        <Section>
          ...
          <p className="text-pretty font-mono text-sm  print:text-[12px]">
            {RESUME_DATA.summary}
          </p>
        </Section>
        <Section>
          ...
          {RESUME_DATA.work.map((work) => {
            return (
              <Card id="card" key={work.company}>
                ...
                <CardContent className="mt-2 py-4 text-xs print:text-[10px]">
                  {work.description}
                </CardContent>
              </Card>
            );
        ...
        <Section>
          ...
          {RESUME_DATA.education.map((education) => {
            return (
              <Card id="education" key={education.school}>
                <CardHeader>
                  <div className="flex items-center justify-between gap-x-2 text-base">
        ...
        <Section>
          ...
          <div className="flex flex-wrap gap-1">
            {RESUME_DATA.skills.map((skill) => {
              return (
                <Badge className="print:text-[10px]" id="skills" key={skill}>
                  {skill}
                </Badge>
              );
            })}
        ...
        <Section className="print-force-new-page scroll-mb-16">
          ...
          {RESUME_DATA.achievements.map((achievement) => {
            return (
              <Card id="card" key={achievement.title}>
                <CardHeader>
                  <div className="flex items-center justify-between gap-x-2 text-base">
                    <h3 className="inline-flex items-center justify-center gap-x-1 font-semibold leading-none">
                      {achievement.title}
